                ...
                <div className="absolute bottom-0 w-full bg-gray-100 bg-opacity-75 opacity-0 group-hover:opacity-100 transition-opacity duration-300 p-4 grid grid-flow-col text-white text-lg font-semibold">
                  <div className="flex flex-wrap justify-start gap-2">
                    {activeColor?.availableSizes
                      .filter(
                        (size) =>
                          size.sizeAvailability === Availability.IN_STOCK
                      )
                      .map((size, index) => (
                        <h1
                          key={index}
                          className="inline-block bg-white hover:bg-gray-100 text-black hover:text-black px-2 py-1 text-base font-semibold mr-2 transition-all duration-300 w-[40px] text-center"
                        >
                          {size.name}
                        </h1>
                      ))}
