...
function LoginPage() {
  ...
  const { mutate, isPending } = useMutation<IAuthResponse, IErrorResponse, IAdmin>({
    mutationKey: ['admin'],
    ...
    onError: (error) => {
      ...
      setErrorMessage(error.status === 400 ? 'Unmatched admin credentials' : 'An unexpected error occurred');
    },
    onSuccess: () => {
      setEmail('');
      setPassword('');
    ...
    <div className="bg-blue-300 flex flex-col items-center justify-center min-h-screen p-6">
      ...
      >
        ...
        <input
          ...
          onFocus={() => { setInputBorder('border-transparent'); setIsErrorShown(false) }}
        />
        ...
        <button
          type="submit"
          className={`p-2 rounded ${isPending ? 'bg-gray-500' : 'bg-blue-500'} text-white`}
          disabled={isPending}
        >
          {isPending ? (
            <span className="flex items-center justify-center">
              <svg className="animate-spin h-5 w-5 mr-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                ...
                <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v1a7 7 0 00-7 7H4zm0 0a8 8 0 018 8v1a7 7 0 00-7-7H4z"></path>
              </svg>
              Logging in...
            </span>
          ) : 'Login'}
